...
def get_stats(cp, conf):
    ...
    orig_gray_mcherry_no_bg, backgroundmcherry = subtract_background_rolling_ball(orig_gray_mcherry, 50,
    ...
    ########################### General Contours ####################################
    def find_ctr(ctr, a, bestCtr, bestA):
    ...
    find_ctr(contours, area, bestContours, bestArea)
    find_ctr(contours1, area1, bestContours1, bestArea1)
    ########################### mcherry contours ####################################
    ...
    ################### general intensity #####################
    def find_mcherry_gfp_intensity(ctr_type, a, bestCtr_type, bestA_type, bestCtr, best_ctr, line_pts):
        for i, cnt, in enumerate(ctr_type[0]):
            if len(cnt) == 0:
                continue
            if i == len(ctr_type[0]) - 1:
                continue
            try:
                a = cv2.contouArea(cnt)
            except:
                continue
            if len(bestCtr_type) == 0:
                bestCtr_type.append(i)
                bestA_type.append(a)
                continue
            if len(bestCtr_type) == 1:
                bestCtr_type.append(i)
                bestA_type.append(a)
            if a > bestA_type[0]:
                bestA_type[1] = bestA_type[0]
                bestA_type[0] = a
                bestCtr_type[1] = bestCtr_type[0]
                bestCtr_type[0] = i
            elif a > bestA_type[1]:
                bestA_type[1] = a
                bestCtr_type[1] = i

        if len(bestCtr_type) == 2:
            c1 = ctr_type[0][bestCtr_type[0]]
            c2 = ctr_type[0][bestCtr_type[1]]
            M1 = cv2.moments(c1)
            M2 = cv2.moments(c2)
        ...
        if len(bestCtr) == 2:
            c1 = contours[bestCtr[0]]
            c2 = contours[bestCtr[1]]
            MERGE_CLOSEST = True
            if MERGE_CLOSEST:  # find the two closest points and just push c2 into c1 there
                ...
                for pt1 in c1:
                    ...
                    while current_loc != finish_loc:
                        best_ctr.append(c2[current_loc])
                        current_loc += 1
                        if current_loc>= len(c2):
                            current_loc = 0
                    best_ctr.append(c2[finish_loc])
                best_ctr = np.array(best_ctr).reshape(-1, 1, 2).astype(np.int32)

    find_mcherry_gfp_intensity(contours_mcherry, area, bestContours_mcherry, bestArea_mcherry,
                               bestContours, best_contour, mcherry_line_pts)
    find_mcherry_gfp_intensity(contours_gfp, area1, bestContours_gfp, bestArea_gfp, bestContours1,
                               best_contour1, gfp_line_pts)
    ...
    # TODO: fix error = OpenCV(4.7.0) :-1:error: (-5:Bad argument) in function 'drawContours'
    # when Clicking on next/prev image/pair.
